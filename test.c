...
void	ft_rotate(t_mlx *mlx, int dir)
{
	...
	{		
		...
		double tz0 = tx0 * cos(mlx->player->angle) + ty0 * sin(mlx->player->angle);
		double tz1 = tx1 * cos(mlx->player->angle) + ty1 * sin(mlx->player->angle);
		// double tz2 = tx2 * cos(mlx->player->angle) + ty2 * sin(mlx->player->angle);

		tx0 = tx0 * sin(mlx->player->angle) - ty0 * cos(mlx->player->angle);
		tx1 = tx1 * sin(mlx->player->angle) - ty1 * cos(mlx->player->angle);
